...
/** Ensures that we can discover New Relic in a Play project. */
class CanDiscoverNewRelicPlay21Sbt12Project extends SbtProcessLauncherTest {
...
/** Ensures that we can discover missing New Relic in a Play project. */
class CanDiscoverMissingNewRelicPlay21Sbt12Project extends SbtProcessLauncherTest {
  val newRelicAgentVersion = "3.5.1"
  val dummy = utils.makeEmptySbtProject("runPlay215", TestUtil.sbt12TestVersion)
  val plugins = new File(dummy, "project/plugins.sbt")
  // TODO - Abstract out plugin version to test more than one play instance...
  ...
  try {
    ...
    Await.result(result.future, timeout.duration) match {
      case RunResponse(success, "run") =>
        ...
        newRelicAgent match {
          case None => // all good
          case Some(_) => throw new AssertionError("Found New Relic when we weren's supposed to")
        }
      case whatever => throw new AssertionError("did not get RunResponse got " + whatever)
    }
  } catch {
